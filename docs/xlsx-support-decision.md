...
目前 repo 能證明的是**一個完整、近期、可重現的專案型案例**。這足以證明需求
存在，也足以證明 read-only 不夠；但不足以宣稱它是日常高頻的通用轉檔需求。
...
| 重複利用潛力 | 高；同類精算、財務、成績與報表工作可共用能力 |
| 長期統計 | 未知；不得由單一案例外推 |

## 為什麼必須是讀寫

| 能力 | Read-only 能否處理 | 本工作流是否需要 |
|---|---:|---:|
...
- 不把 genoffice 的完整 spreadsheet engine 列為本專案的必備同等功能。
