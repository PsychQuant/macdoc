# ooxml-paragraph-text-mirror Specification

## Purpose

Defines the shared text universe between paragraph read APIs and text replacement mutators. `flattenedDisplayText()` and `replaceInParagraphSurfaces` must walk the same paragraph surfaces, expose direct-child OMML visible text for anchor lookup, and refuse replacements that would cross OMML boundaries instead of silently creating misleading Word content.
