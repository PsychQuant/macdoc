# xml-attribute-escape Specification

## Purpose

Defines a single shared XML attribute escape helper for `ooxml-swift` and the emit-site routing requirements that prevent caller-provided String values from injecting raw OOXML markup into attribute positions. Replaces the prior pattern of duplicated `fileprivate` escape helpers scattered across emit sites.
