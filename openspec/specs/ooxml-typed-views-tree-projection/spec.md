# ooxml-typed-views-tree-projection Specification

> **Related**: this capability is the sibling-type extension of `ooxml-paragraph-tree-projection` (which covers `Paragraph`). Together they implement the typed-view-as-tree-projection pattern for all five Phase 1 typed views in scope for `word-aligned-state-sync` (`Paragraph`, `Run`, `Table`, `TableRow`, `TableCell`, `SectionProperties`). The remaining Phase 1 typed view `Settings` has no corresponding struct in the codebase yet — follow-up changes `settings-extraction-impl` (extracts the struct from `Document.swift`) → `settings-tree-projection-impl` (applies the same pattern). Full `<w:sectPr>` structured-field tree-walking for `SectionProperties` is deferred to follow-up `section-properties-tree-walking-impl`.

## Purpose

Defines the contract for tree-backed views over `<w:r>`, `<w:tbl>`, `<w:tr>`, `<w:tc>`, and `<w:sectPr>` `XmlNode`s in `ooxml-swift` (`packages/ooxml-swift/Sources/OOXMLSwift/Models/Run.swift`, `Table.swift`, `Section.swift`). Tree-backed `Run`, `Table`, `TableRow`, `TableCell`, and `SectionProperties` each wrap an existing `XmlNode` from the lossless DOM (`ooxml-tree-io` capability, landed v0.30.0) and expose typed view accessors that walk the underlying tree at every access rather than caching a snapshot.

This capability covers, for each of the five sibling typed views:

1. **Constructor surface** — a new `Type(xmlNode:)` entry point co-exists with the existing legacy constructors; both produce values of the same struct type interchangeable across the public API surface
2. **Identity model** — `value.id: String?` derives from `XmlNode.stableID` (e.g. `"w:id=42"` for a `<w:r>`) with a `"lib:<UUID>"` fallback when only `libraryUUID` is set; returns `nil` for detached values
3. **Getter routing** — primary content accessors (`Run.text`, `Table.rows`, `TableRow.cells`, `TableCell.paragraphs`, `TableCell.nestedTables`) are mode-aware computed properties that walk `xmlNode.children` at every access in tree-backed mode and read from legacy stored fields in detached mode
4. **Setter routing (Phase 1 stub)** — `Run.text` setter mutates the wrapped xmlNode directly (replaces `<w:t>` direct children with one new `<w:t>X</w:t>` while preserving `<w:rPr>`/`<w:tab>`/`<w:br>` siblings) and calls `markDirty()`; setters for `rows`/`cells`/`paragraphs`/`nestedTables` are Phase 1 ghost-writes to legacy buffers (op-log routing arrives in Phase 2)
5. **Equality semantics** — identity-based for two tree-backed values of the same type (`xmlNode === xmlNode`); content-based for two detached values; always `false` across mixed modes
6. **SectionProperties Phase 1 stub** — tree-backed `SectionProperties` is identity-only in Phase 1; the 12+ structured fields (`pageSize`, `pageMargins`, `orientation`, `columns`, `docGrid`, `headerReferences`, `footerReferences`, `lineNumbers`, `verticalAlignment`, `pageNumberFormat`, `pageNumberStartValue`, `titlePageDistinct`, `sectionBreakType`) remain as legacy stored properties; tree-backed reads return the `SectionProperties()` defaults
7. **Public API preservation** — the refactor is strictly additive on the consumer side; no method, property, initializer, or conformance is renamed, removed, or has its signature altered on any of the five types
8. **GREEN-from-the-start tests** — three new test files (`RunTreeProjectionTests`, `TableTreeProjectionTests`, `SectionPropertiesTreeProjectionTests`) land GREEN against the implementation in the same release (no `#if false`-gated RED scaffold phase, unlike `paragraph-tree-projection-impl` which originally landed RED in commit `c97de51` then went GREEN in v0.31.0)

The capability does not own the tree itself (that's `ooxml-tree-io`), the operation log (Phase 2 of `word-aligned-state-sync`), the `Paragraph` view (that's `ooxml-paragraph-tree-projection`), or the `mdocx` DSL (`mdocx-grammar`). It is the narrow contract for "Run / Table family / SectionProperties as typed views over the tree."
...
<!-- @trace
source: sibling-types-tree-projection-impl
updated: 2026-05-07
code:
