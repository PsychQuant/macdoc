# word-mcp-markdown-export Specification

## Purpose

MCP tools in `che-word-mcp` that export manuscript review artifacts as markdown — per-doc revision summary, cumulative cross-version timeline, comment thread view with alias normalization. Defines the `summarize: Bool = false` truncation policy shared across all che-word-mcp tools that return potentially long text.
