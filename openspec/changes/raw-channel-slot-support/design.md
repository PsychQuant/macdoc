...
1. **Validation fallback in `exportSwift`, not reverse-side upgrade.** When the `.appendParagraph` scan misses, locate the paraId inside the `word/document.xml` carryPart XML with a **structure-aware scanner** — a single linear pass that is quote-aware inside attribute values, skips comments/CDATA/PIs, tracks element nesting depth, and recognizes `w14:paraId` only as an attribute of a `<w:p>` opening tag. Bare token search is forbidden: Word legitimately writes `w14:paraId` on `<w:tr>` elements (20 of REC-O-01's 115), the literal token can appear inside `<w:t>` text content or a slot value, and `<w:p>` nests through `<w:txbxContent>` — all three break lexical matching (verify round 1, findings #1/#7/#17). Why not reverse-side: touching `documentUpgrade` risks the byte-equal foundation for every document, slotted or not; the fallback is scoped to slot designation only.
...
5. **All ambiguity fails loudly, at export AND at import.** Three designation refusals (closed list): absent from both domains; carried by more than one `<w:p>`; present but not on any `<w:p>` (e.g. a `<w:tr>` id — the error names the actual carrier). Import re-applies the guards (unknown directive name, unlocatable paraId, duplicate) and verifies post-surgery well-formedness with a real XML parse, failing loudly instead of writing a corrupt part; slot values containing characters forbidden by XML 1.0 are refused. Why: strict mode already refuses ambiguity; silently picking the first occurrence — or silently skipping a stale directive — risks filling the wrong cell of an official form, or shipping an unfilled one that is byte-equal to a correct default run (verify round 1, findings #5/#10).
