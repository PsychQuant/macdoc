...
Implements the `ooxml-script-transcode` capability.

- [ ] 5.1 Implement `ScriptExporter.exportSwift(log:)` covering the **operation log to Swift script export** requirement
- [ ] 5.2 [P] Implement `ScriptImporter.parse(source:)` covering the **Swift script to operation log import** requirement, including structured error reporting (`TranscodeError.unsupportedSyntax`)
- [ ] 5.3 [P] Cover the **build a docx end-to-end from a Swift script** requirement: `Document.create()` produces a valid empty docx; round-trip through Word save preserves structural equivalence
...
- [ ] 5.5 [P] Cover the **script export covers all operation types in the log** requirement, including the unknown-op_type forward-compat case
- [ ] 5.6 Round-trip integration test: log → script → log produces equivalent operations (same `op_type` and `payload` per position; `op_id` and `timestamp` may regenerate)
- [ ] 5.7 Tag and release `ooxml-swift v0.34.0`
