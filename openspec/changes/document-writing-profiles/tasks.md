...
- [x] 1.2 實作 Shared explicit OOXML API、Official formatting 與 Inherit preserves document-owned formatting；以普通 writer／authoring writer 的 styles/theme/section 測試驗證 twips 值一致、新文件無强制字型、既有文件不變。
...
- [x] 2.3 將 Profile selection 接到 Word MCP create/open/execute 與 CLI render；測試新建默认、既有/replay 無 explicit 不變、失敗不註冊 session／不發布 output。
