...
- [x] 4.1 Update `docs/structural-editing-paradigm.md` §3.1 to record new sizeLossRatio achievement (16.66% → ≤ 5%)
- [x] 4.2 Promote §6.1「edit 一個字 → document.xml shrinks <1%」demo from "deferred" to live (caveat removed) once §3.5.5 marketplace sync confirms
- [x] 4.3 Update §10 to list TWO invariants with paragraph-level coverage explicit (replaces sub-stack-C-era「partial run-level coverage」note)
- [x] 4.4 Commit doc update separately on macdoc main with message linking to closed #65 and #66

## 5. Issue closure

- [x] 5.1 Close che-word-mcp #65 via `idd-close` after sub-stack D ships and matrix-pin ratchet lands
- [x] 5.2 Close che-word-mcp #66 via `idd-close` after sub-stack E ships and matrix-pin ratchet lands
- [x] 5.3 Cross-link both closures back to this Spectra change archive directory once `/spectra-archive` runs
