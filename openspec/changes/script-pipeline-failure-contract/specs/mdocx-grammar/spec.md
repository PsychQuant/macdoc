...
The command SHALL expose an overwrite flag and SHALL pass it to the shared entry point rather than enforcing an overwrite check of its own. The refusal to clobber an existing output file, and the guarantee that a failed verification leaves the output path untouched, are properties of that entry point; the command SHALL NOT duplicate them.

The command SHALL report a completed write only after the verification outcome is known. When verification fails, the command SHALL NOT report that a document was written.
...
- **AND** the command exits with a failure status only if the rebuild itself failed

#### Scenario: Verification mismatch is reported and fails without writing
...
- **AND** the command does not report that a document was written
- **AND** the output path holds what it held before the run, or holds nothing if it held nothing
...
#### Scenario: The overwrite flag reaches the shared entry point

- **GIVEN** a file already present at the requested output path
- **WHEN** `macdoc word render` runs with the overwrite flag
- **THEN** the rebuild proceeds
- **AND** the refusal that would otherwise occur originates from the shared entry point rather than from a check inside the command
