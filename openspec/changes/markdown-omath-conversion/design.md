...
The converter first parses the original source with `swift-markdown` and converts source ranges into character offsets using the parser's UTF-8 line/column semantics. Text-node ranges define where inline math is eligible. Original InlineCode/CodeBlock/HTML/Link/Image ranges build an explicit opaque bitmap; for parser-recognized multiline links whose reported range cannot map back to raw columns, the decoded destination is recovered only inside the enclosing original paragraph continuation and marked opaque. Recovery uses a per-paragraph monotonic cursor and a linear prefix matcher, so many fallback links neither rescan all paragraph ranges nor copy overlapping suffixes. When cmark normalizes visible text and reports a child range that does not cover its raw spelling, the scanner recovers only exact dollar-span signatures present in that normalized `Text`, matched from the suffix of the same plain-text paragraph. Display closings likewise require a positively eligible `Text` offset; exact `$$` signatures are recovered for normalized plain-text paragraphs without making the whole paragraph or a metadata prefix eligible. Normalized candidate pairing advances delimiter-to-delimiter rather than rescanning suffixes; the raw pairing pass skips backslash-escaped characters in one forward traversal, so odd or even backslash spellings cannot create repeated candidate openers. A bounded raw pairing pass may mask an opening HTML spelling only when an original `InlineHTML` closing node anchors the same tag, the complete opening spelling satisfies the CommonMark tag-and-attribute grammar, and the recovered dollar text lies inside a quoted attribute. The pass advances monotonically to a discovered tag or physical-line boundary so malformed prefixes cannot trigger suffix rescans. Unpaired or grammatically invalid HTML-like text remains governed by its original `Text` range and stays math-eligible. Paragraph ranges whose descendants contain only plain text and line-break nodes define where display math is eligible. Code, HTML, autolink, destination, reference metadata, and formatting-container ranges are therefore opaque without making a free-standing raw angle heuristic override the CommonMark tree.

The scanner then performs a bounded linear replacement pass over source characters. Physical line/content ends and a global non-whitespace prefix-count are computed once, so every delimiter can test prefix/suffix whitespace in O(1), including opaque code/HTML/destination delimiters and Unicode-whitespace-prefixed lines. It recognizes:
